...
class BaseLMDB(Dataset):
    ...
    def __getitem__(self, index):
        with self.env.begin(write=False) as txn:
            key = f'{str(index).zfill(self.zfill)}'.encode(
                'utf-8')
...
class FFHQ1024lmdb(Dataset):
    def __init__(self,
                 path=os.path.expanduser('datasets/ffhq1024.lmdb'),
                 image_size=1024,
...
class NATURElmdb(Dataset):
    def __init__(self,
                 path=os.path.expanduser('datasets/nature2560.lmdb'),
                 image_size=(512,1280),  
                 original_resolution=(1024,2560),
                 split=None,
                 as_tensor: bool = True,
                 do_augment: bool = False,
                 do_normalize: bool = True,
        ...
        if do_augment:
            transform.append(transforms.RandomHorizontalFlip())
        if do_crop:
            transform.append(transforms.CenterCrop((512,1024)))
        if as_tensor:
            transform.append(transforms.ToTensor())
        if do_normalize:
            transform.append(
...
class LHQ1024lmdb(Dataset):
    def __init__(self,
                 path=os.path.expanduser('../datasets/lhq1024.lmdb'),
                 image_size=1024,
                 original_resolution=1024,
                 split=None,
                 as_tensor: bool = True,
...
class Church_lmdb(Dataset):
    def __init__(self,
                 path=os.path.expanduser('datasets/church256.lmdb'),
                 image_size=256,
                 original_resolution=256,
                 do_augment: bool = False,
                 do_transform: bool = True,
