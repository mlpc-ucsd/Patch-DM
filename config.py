    ...
    def make_dataset(self, path=None, **kwargs):
        if 'ffhq256' in self.data_path:
            ...
            return Horse_lmdb(path=path or self.data_path,
                              image_size=256,
                              **kwargs)
        elif 'church' in self.data_path:
            return Church_lmdb(path=path or self.data_path,
                              image_size=256,
                              **kwargs)
        elif 'ffhq1024' in self.data_path:
            return FFHQ1024lmdb(path=path or self.data_path,
                            image_size=1024,
                            **kwargs)
        elif 'nature' in self.data_path:
            return NATURElmdb(path=path or self.data_path,
                            image_size=(512,1280),
                            do_crop=True,
                            **kwargs)
        elif 'lhq' in self.data_path:
            return LHQ1024lmdb(path=path or self.data_path,
                            image_size=1024,
                            **kwargs)
        else:
            return None
